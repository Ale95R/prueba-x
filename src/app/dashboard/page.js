...
/**
 * Persistencias existentes en tu app:
 * - Usuarios: localStorage key "pm_users" (se usa abajo para listar y asignar)
 * - Sesión:   localStorage key "pm_session" (la maneja tu AuthContext)
 *
 * Proyectos: AHORA vienen del mock backend (json-server) vía src/lib/api.js
 * Shape en el FRONT (lo conservamos):
 * {
 *   id: number,
 *   title: string,
 *   description: string,
 *   duration: string,
 *   phases: [{ id:number, name:string, doneBy:string[] }],
 *   members: string[]  // emails asignados
 * }
 */
...
export default function DashboardPage() {
  ...
  async function onSubmitProject(e) {
    ...
    try {
      if (editingId) {
        await apiPatchProject(editingId, base); // mantiene members
        setEditingId(null);
      } else {
    ...
    <Protected>
      ...
      <RoleGate allow={["manager"]}>
        <section style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8, marginTop: 12 }}>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (confirm(`¿Terminar/eliminar el proyecto "${p.title}"?`)) {
                            try {
                              await apiDeleteProject(p.id); // soft-delete en middleware
          ...
          {/* --- TAB: Asignar tareas (proyectos a usuarios) --- */}
          {activeTab === "assign" && (
            <div className="card">
      ...
      <style jsx>{`
        .card {
          ...
          margin-top: 12px;
          background: #fff;
          color: #111;           /* texto de la card en negro */
          color-scheme: light;   /* controles en esquema claro por defecto */
        }
        ...
        /* Inputs/textarea/select oscuros dentro de la card */
        .card .grid input,
        .card .grid textarea,
        .card .grid select {
          background: #111;          /* caja oscura */
          color: #fff;               /* texto blanco */
          border: 1px solid #374151; /* gris oscuro */
        }
